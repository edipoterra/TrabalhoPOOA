 ...
 */
public class gui_GraficoEvEquipe extends JDialog {

    public Connection conn;

    /**
     * construtor da classe graficosEvEquipe
     * @param conn
     * @throws Exception
     */
    public gui_GraficoEvEquipe(Connection conn) throws Exception{
        this.conn = conn;
    ...
    private XYDataset createDataset() throws Exception{
        ...
        for (int i = 0; i < dados[0].length; i++){
            if (dados[0][i].equals(aux)){
                series.add(i, Integer.parseInt(dados[3][i]));
            }
            else{
                if (i > 0){
